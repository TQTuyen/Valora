...
import type { ReactFieldBindings, ReactFieldState, ReactFormState } from './types';
...
export class ReactAdapter<T extends Record<string, unknown>> extends BaseFrameworkAdapter<T> {
  ...
  useForm(): ReactFormState<T> {
    ...
    const canSubmitValue = useMemo(
      () => canSubmit(formState),
      [formState.isValid, formState.validating],
    );
  ...
  getFieldBindings<K extends keyof T>(field: K): ReactFieldBindings<T[K]> {
