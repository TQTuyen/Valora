/**
 * Less Than Or Equal Strategy
 * @module validators/comparison/strategies/less-than-or-equal
 */
...
/** Less than or equal strategy */
export class LessThanOrEqualStrategy<T extends number | Date> extends BaseValidationStrategy<T, T> {
  ...
  validate(value: T, context: ValidationContext): ValidationResult<T> {
    ...
    if (typeof numValue !== 'number' || typeof numCompare !== 'number' || numValue > numCompare) {
      ...
      return this.failure('comparison.lessThanOrEqual', context, { value: fieldName });
    }
