  ...
  <div class="comprehensive-demo">
    ...
    <section class="form-section">
      ...
      <p class="section-description">
        Showcasing complex validation rules, form state subscriptions, and reactive UI updates
      </p>
      ...
      <form @submit.prevent="handleSubmit" class="demo-form">
        ...
        <div class="form-group">
          ...
          />
          <small class="hint">Lowercase letters, numbers, and underscores only</small>
        ...
        <div class="form-group">
          ...
          />
          <small class="hint">Vietnamese phone number (10 digits, starts with 0)</small>
      ...
      <form @submit.prevent="handleContactSubmit" class="demo-form">
        ...
        <div class="form-group">
          ...
          <input
            ...
            :class="{ 'error': contactName.shouldShowError.value, 'success': contactName.isValid.value && contactName.touched.value }"
          />
          <div v-if="contactName.shouldShowError.value" class="error-messages">
            <p v-for="(msg, index) in contactName.errorMessages.value" :key="index" class="error-message">
              {{ msg }}
            </p>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <input
            ...
            :class="{ 'error': contactEmail.shouldShowError.value, 'success': contactEmail.isValid.value && contactEmail.touched.value }"
          />
          <div v-if="contactEmail.shouldShowError.value" class="error-messages">
            <p v-for="(msg, index) in contactEmail.errorMessages.value" :key="index" class="error-message">
              {{ msg }}
            </p>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <textarea
            ...
            :class="{ 'error': contactMessage.shouldShowError.value, 'success': contactMessage.isValid.value && contactMessage.touched.value }"
          ></textarea>
          <!-- Character Counter (using field subscription) -->
          <small class="hint" :class="characterCounterClass">{{ characterCounterText }}</small>
          <div v-if="contactMessage.shouldShowError.value" class="error-messages">
            <p v-for="(msg, index) in contactMessage.errorMessages.value" :key="index" class="error-message">
              {{ msg }}
            </p>
          </div>
        </div>
...
const passwordHint = ref('Minimum 8 characters with uppercase, lowercase, number, and special char');
...
adapter.subscribeToField('password', (fieldState) => {
  ...
  } else if (fieldState.touched && fieldState.errors.length === 0 && fieldState.value) {
    passwordHint.value = '✓ Strong password!';
  } else {
    passwordHint.value = 'Minimum 8 characters with uppercase, lowercase, number, and special char';
  }
});
...
contactAdapter.subscribeToForm((state) => {
  canSubmitContactForm.value = state.touched && state.isValid && !state.validating;
